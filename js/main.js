...
$(document).ready(function(){
  ...
  $('#qty').on('change keyup', function() {
      var qty=parseInt($(this).val()), priceForOne=parseInt($('#priceForOne').val());
  ...
  $('.ulduz').click(function(){
      ...
      for (let index = 0; index <= necenci; index++) {
          $('.ulduz'+index).addClass('bi-star-fill').removeClass('bi-star');
          
      }

      for (let index = necenci+1; index <= 5; index++) {
          $('.ulduz'+index).addClass('bi-star').removeClass('bi-star-fill');
  ...
  $('#product-add').on('submit', function(){
      var productName=$('#prd-name').text(), qty=parseInt($('#qty').val()), term=0;
      var productID=$('.product-head').data('id'), priceForOne=parseInt($('#priceForOne').val());
      var totalPrice=(priceForOne*qty).toFixed(2);
      ...
      if($('#'+productFullID).length){
          $('#'+productFullID+' span').text('$'+totalPrice+'('+qty+') qty') ;
          alert('Product details refreshed');
      }
      else {
          $('.shop-list').append('<li id="'+productFullID+'"><a href="#" class="dropdown-item">'+productName+' <span class="badge bg-danger">$'+totalPrice+'('+qty+') qty</span></a></li>');
